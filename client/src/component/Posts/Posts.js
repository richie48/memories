...
const Posts = ({setCurrentId}) => {
  ...
  return (
    ...
    <h1 className="text-center font-bold mt-4">Posts</h1>
      {!posts.length? <h1 className="text-center mt-10">loading...</h1> : <div className="grid lg:grid-cols-3 md:grid-cols-2 " >{posts.map((post) => <Post post={post} key={post._id} setCurrentId={setCurrentId}/>)}</div>}
    </>)
};
